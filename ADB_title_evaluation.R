...
mergedPCR <- merge(pcr, topicProbabilities, by="ID")
write.csv(mergedPCR, file = paste("output\\PCR_with_topics.csv"))


#Now run using LDA and LDAVis
#Change the PCR variables below to the pre-processed ones... 
ldaInput <- setNames(sapply(1:length(corpus), function(i){corpus[[i]]$content}), pcr[["ID"]])
...
stmSelectK <- searchK(stmData$documents, stmData$vocab, K = seq(5, 15),
                      prevalence =~  I..CHOOSE.SECTOR, data = stmData$meta, init.type = "Spectral")
